    ...
    public partial class Form1 : Form
    {
        ...
        void sp1_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            if (sp1.IsOpen)     //此处可能没有必要判断是否打开串口，但为了严谨性，我还是加上了
            {
                ...
                {
                    txtReceive.Text += sp1.ReadLine() + "\r\n"; //注意：回车换行必须这样写，单独使用"\r"和"\n"都不会有效果首先，应该在接收之前加一个延时，给计算机时间接收数据，不然会导致接收的数据显示不全。 解决方法：可以在//输出当前时间代码前加：System.Threading.Thread.Sleep(100); 其次，如果下位机一次性发送了好几行数据，但是只会显示第一行。 解决方法：不使用ReadLine();修改如下： if (rbRcvStr.Checked) //'发送字符串'单选按钮 { //方式一 可实现中文接收 byte[] readByte = new byte[sp1.BytesToRead]; sp1.Read(readByte, 0, readByte.Length); string s = Encoding.Default.GetString(readByte); txtReceive.Text +=s + &quot;\r\n&quot;; //方式二 // txtReceive.Text += sp1.ReadExisting() + &quot;\r\n&quot;; // sp1.DiscardInBuffer(); //清空SerialPort控件的Buffer }
                    sp1.DiscardInBuffer();                      //清空SerialPort控件的Buffer 
                }
                else                                            //'发送16进制按钮'
                {
